...
public class LoginInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(StringUtils.isEmpty(token)){
			...
			System.out.println("step2 "+SSO_URL+"/sso/login?redirect="+request.getRequestURL().toString());
			return false;
		}
		...
		if(result.getStatus()!=200){
			//5.用户已经过期  --》重定向到登录的页面
			response.sendRedirect(SSO_URL+"/sso/login?redirect="+request.getRequestURL().toString());
			System.out.println("step3 "+SSO_URL+"/sso/login?redirect="+request.getRequestURL().toString());
			return false;
		}
		//6.用户没过期（说明登录了）--》放行
